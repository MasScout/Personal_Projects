...
// Author: Mason Barrow (adapted from Jacob Hauenstein (adapted from Rick Coleman's work))
// Date:   April 2020
//-------------------------------------------------------------------------
...
int Hash_1(char *key)
{
	...
	return ((key[0] * key[1]) + key[2] * key[3]) % TABLESIZE;
}
...
int Hash_2(char *key)
{
	//Using the Middle Squaring Function
	//return ((key[1] + key[2]) ^ 2) % TABLESIZE;
	return (int)(((key[0] - 'A')*pow(26, 3)) + ((key[1] - 'A')*pow(26, 2)) + ((key[2] - 'A') * 26) + ((key[3] - 'A'))) % TABLESIZE;
}
...
int Hash_3(char *key)
{
	...
	//return (key[0] * key[1]) % TABLESIZE;
	return (key[2] * key[3]) % TABLESIZE;
}
